...
try {
    ...
    // 9. Record blockchain entry using BlockchainService
    require_once __DIR__ . '/../../src/lib/BlockchainService.php';
    $blockchain = new \App\Lib\BlockchainService($supabase);
    
    $blockchain->record('payment', $affiliationResult['id'] ?? null, [
        'pending_affiliation_id' => $pendingAffiliationId,
        'payment_status' => 'verified',
        'total_fee' => $affiliationResult['total_fee'] ?? null,
        'receipt_number' => $affiliationResult['receipt_number'] ?? null,
        'verified_by' => $_SESSION['user']['email'] ?? 'system',
        'verified_at' => date('c')
    ]);
