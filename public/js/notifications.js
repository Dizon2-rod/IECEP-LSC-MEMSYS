...
class NotificationManager {
    ...
    setupRealtimeUpdates() {
        // Use IECEP_REALTIME API for Supabase subscriptions
        if (window.IECEP_REALTIME) {
            const userId = window.currentUserId || '';
            const filter = userId ? 'user_id=eq.' + userId : null;

            window.IECEP_REALTIME.subscribe(
                'notifications',
                'INSERT',
                (payload) => {
                    if (!window.IECEP_REALTIME.validatePayload(payload, ['id', 'user_id'])) return;
            ...
            // Fallback: wait for realtime engine to be ready
            window.addEventListener('iecep:realtime:ready', () => {
                this.setupRealtimeUpdates();
            }, { once: true });
        }
    }
