...
<body>
    ...
    <script>
        ...
        // Current school officer institution id from session
    ...
    <script>
    /**
     * School Officer Dashboard — Realtime block
     * Subscribes to: members (filtered by institution_id), attendance
     * Updates: Total Members card, Attendance Rate card.
     */
    (function () {
        'use strict';
        ...
        function setCard(index, value) {
            ...
            if (cards[index]) {
                ...
                if (window.IECEP_REALTIME) {
                    IECEP_REALTIME.animateBump('.stat-card:nth-child(' + (index + 1) + ') .stat-content h3');
                }
            }
        }
        ...
        function boot() {
            ...
            const instId = (window.IECEP_SCHOOL_OFFICER || {}).institutionId || '';

            // — Subscribe to members for this institution ———————
            // If institutionId is available, use a server-side filter.
            const memberFilter = instId ? 'institution_id=eq.' + instId : null;

            RT.subscribe('members', ['INSERT', 'DELETE'], (payload) => {
                if (!RT.validatePayload(payload, ['id'])) return;
                // Only care about this institution if filter isn't supported
                if (instId && payload.new && payload.new.institution_id !== instId) return;
                if (instId && payload.old && payload.old.institution_id !== instId) return;

                _memberCount += payload.eventType === 'INSERT' ? 1 : -1;
                _memberCount = Math.max(0, _memberCount);
                setCard(0, _memberCount);
                RT.showToast(
                    payload.eventType === 'INSERT' ? 'New member added' : 'Member removed',
                    'info'
                );
            }, memberFilter);

            // — Subscribe to attendance for compliance rate refresh —
            RT.subscribe('attendance', ['INSERT', 'UPDATE', 'DELETE'], (payload) => {
                if (!RT.validatePayload(payload, ['id'])) return;
                // Re-fetch attendance summary
                RT.refetchWithRetry(
                    '/IECEP-LSC-MEMSYS/public/api/school-officer/financial.php?summary=1',
                    (data) => {
                        if (data && data.attendance_rate !== undefined) {
                            setCard(1, data.attendance_rate + '%');
                        }
                    },
                    'so-attendance'
                );
            ...
            // — Load initial member count via API ———————————
            if (instId) {
                RT.refetchWithRetry(
                    '/IECEP-LSC-MEMSYS/public/api/member.php?action=count&institution_id=' + encodeURIComponent(instId),
                    (data) => {
                        if (data && data.count !== undefined) {
                            _memberCount = parseInt(data.count) || 0;
                            setCard(0, _memberCount);
                        }
                    },
                    'so-member-count'
