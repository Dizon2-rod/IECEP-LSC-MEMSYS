-- Simple test user creation - most reliable method
...
INSERT INTO auth.users (
  ...
  NOW(),
  NOW(),
  '{"full_name": "Test User", "role": "member"}'
);
...
-- Step 3: Create member record (assuming UPLB institution exists)
INSERT INTO members (institution_id, user_id, full_name, email, member_type, payment_status, year_level)
VALUES (
  (SELECT id FROM institutions WHERE email = 'admin@uplb.edu.ph' LIMIT 1),
  (SELECT id FROM auth.users WHERE email = 'test@iecep-lsc.org'),
  'Test User',
  'test@iecep-lsc.org',
  'new',
