-- =====================================================================
-- IECEP-LSC MEMSYS - COMPLETE SUPABASE POSTGRESQL QUERY
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'fee_brackets' AND column_name = 'annual_fee'
    ) THEN
        ALTER TABLE fee_brackets ADD COLUMN annual_fee DECIMAL(10,2);
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'fee_brackets' AND column_name = 'is_active'
    ) THEN
        ALTER TABLE fee_brackets ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'fee_brackets' AND column_name = 'created_at'
    ) THEN
        ALTER TABLE fee_brackets ADD COLUMN created_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'fee_brackets' AND column_name = 'updated_at'
    ) THEN
        ALTER TABLE fee_brackets ADD COLUMN updated_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
    
    -- Ensure unique constraint on bracket_name exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'fee_brackets' AND constraint_type = 'UNIQUE'
    ) THEN
        ALTER TABLE fee_brackets ADD CONSTRAINT fee_brackets_bracket_name_key UNIQUE (bracket_name);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'compliance_rules' AND column_name = 'description'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'compliance_rules' AND column_name = 'threshold'
    ) THEN
        ALTER TABLE compliance_rules ADD COLUMN threshold NUMERIC(5,2);
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'compliance_rules' AND column_name = 'created_at'
    ) THEN
        ALTER TABLE compliance_rules ADD COLUMN created_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS user_profiles (
    ...
    role TEXT NOT NULL CHECK (role IN ('admin', 'school_officer', 'member')),
    full_name TEXT,
    school_name TEXT,
    contact_phone TEXT,
    address TEXT,
...
-- Storage policies for two-role system
DROP POLICY IF EXISTS "Registration Committee upload pending" ON storage.objects;
CREATE POLICY "Registration Committee upload pending" ON storage.objects
FOR ALL
TO authenticated
USING (
  (bucket_id = 'documents') AND 
  (auth.uid()::text = (storage.foldername)) AND
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_id = auth.uid()::text 
    AND role IN ('admin', 'school_officer')
  )
);
...
DO $$
BEGIN
    RAISE NOTICE 'IECEP-LSC MEMSYS Supabase Complete Query - Completed at %', now();
END $$;
